...
wdi.Utils = {
    generateWebSocketUrl: function(protocol, host, port, destHost, destPort, type, destInfoToken) {
        ...
        if ( ! destHost || ! destPort ) {
            url = protocol + '://' + host + ':' + port + window.location.pathname + '/websockify/destInfoToken/' + destInfoToken + '/type/' + type;
        } else {
            url = protocol + '://' + host + ':' + port + window.location.pathname + '/desktop/websockify/host/' + destHost + '/port/' + destPort + '/type/' + type;
        }
        return url;
    }
};
